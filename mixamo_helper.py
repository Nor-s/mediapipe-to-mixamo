...
def json_one_frame_to_360_gif(json_object, frame_idx, save_path = '.'):
    ...
    frame = get_mixamo_json_keypoints_with_color_and_mark(json_object, frame_idx)
    ...
    ani.save(save_path + '/' + ntpath.basename(json_object['fileName']) +'_json_mm_1frame.gif', writer='pillow')
...
def json_to_gif(json_object, save_path = '.', max_frame_num = 100, is_axes_move = False):
    ...
    if save_path[-1] == '/':
        save_path = save_path[0: -1]
    if is_axes_move:
        ani.save(save_path + '/' + ntpath.basename(json_object['fileName']) +'_json_mm_round_.gif', writer='pillow')
    else:
        ani.save(save_path + '/' + ntpath.basename(json_object['fileName']) +'_json_mm_.gif', writer='pillow')

def get_mixamo_name_mediapipe_name_map():
    mixamo_name_mediapipe_name_map = {}
    for idx in range(6, len(mixamo_names)):
        mixamo_name_mediapipe_name_map[mixamo_names[idx][0]] = mixamo_names[idx][3] 
